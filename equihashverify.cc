...
void Verify(const v8::FunctionCallbackInfo<Value>& args) {
    ...
    if (!args[3]->IsInt32() || !args[4]->IsInt32()) {
        isolate->ThrowException(
            Exception::TypeError(
                String::NewFromUtf8(isolate, "Fourth and fifth parameters should be equihash parameters (n, k)").ToLocalChecked())
        );
    ...
    if(!node::Buffer::HasInstance(header) || !node::Buffer::HasInstance(solution)) {
        isolate->ThrowException(
            Exception::TypeError
            (String::NewFromUtf8(isolate, "First two arguments should be buffer objects.").ToLocalChecked())
        );
    ...
    if (!args[2]->IsString()) {
        isolate->ThrowException(
            Exception::TypeError(
                String::NewFromUtf8(isolate, "Third argument should be the personalization string.").ToLocalChecked())
        );
